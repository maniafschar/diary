	...
	star(no, x) {
		...
		if (x > 0 || no) {
			...
			if (x < 100) {
				...
				clippath = `<clipPath id="fillClip${id}"><rect x="0" y="0" width="${x * 24 / 100}" height="24" /></clipPath>`;
				clippathAttribute = ` clip-path="url(#fillClip${id})"`;
			}
			fill = `${clippath}<path fill="rgb(210, 225, 20)"${clippathAttribute} d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"></path>`;
		}
		return `<svg width="24" height="24" viewBox="-1 -1 18 18"${no ? ' onclick="this.getRootNode().host.rate(' + no + ', true)"' : ''}>
					${fill}
					<path fill="none" stroke="#bbb" stroke-width="0.5" stroke-linejoin="round" d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"></path>
				</svg>`;
	}
}
