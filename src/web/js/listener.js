			...
			list.push({
				...
				description: '<date' + addEdit() + '>' + ui.formatTime(new Date(events[i].date.replace('+00:00', ''))) + '</date>' +
					...
					(api.user?.id ?
						...
						'<label>Teilnehmer</label><field style="min-height: 3.2em; max-height: initial;">' + listParticipants(events[i]) + '</field>' +
...
img.speak {
	...
	static updateEvents() {
		api.event.getList(events => {
			...
			var rating = [];
			var text = [];
			var ratingSum = 0, ratingCountSum = 0, ids = [];
			...
			for (var i = events.length - 1; i >= 0; i--) {
				...
				if (events[i].rating) {
					...
					if (rating.length == 0 || rating[rating.length - 1].date.substring(0, 10) != events[i].date.substring(0, 10)) {
						rating.push({ ids: ids, date: events[i].date, rating: ratingSum / ratingCountSum });
						ratingSum = 0;
						ratingCountSum = 0;
						ids = [];
					}
