...
class action {
	...
	static loginVerify(contact) {
		api.contact.patch(contact, () => {
			api.authentication.getVerify(contact.email, e => {
				if (e == 'ok') {
					document.querySelector('user view-table').table().querySelector('td[contact*="\\"id\\":' + contact.id + ',"]').innerText = '...';
	...
	static summaryDisplay(summary) {
		document.dispatchEvent(new CustomEvent('popup', {
			detail: {
				body:
					(summary.image ? (summary.image.indexOf('.') > 0 ? '/med/' + summary.image : '<img src="data:image/jpg;base64,' + summary.image + '" style="max-width: 100%;"/>') : '') +
