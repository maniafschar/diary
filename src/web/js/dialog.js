...
${dialog.stylePictures}`;
		...
		var inputDate = dialog.createField(element, 'Datum', 'date', 'input-date', event?.year ? InputDate.local2server(event.year + '-' + event.month + '-' + event.day + ' ' + new Date().getHours() + ':00') : null);
		var date = new Date();
		...
		buttonToggle.setAttribute('style', 'position: absolute; cursor: pointer; background: var(--background-clickable); color: white; top: 0.31em; right: 0.31em; border-radius: 0 0.3em 0.3em 0; width: 1.32em; height: 1.25em; font-size: 1.6em; text-align: center; padding: 0 0.16em;');
